...
class Agent(object):
    ...
    def generateFramesForGraphics(self, frame_rate):
        # For each task...
        # Does not contain the initial task (task 0).
        for i in range(len(self.tasks)):
            if (i == 0):
                # TODO / NOTE Assumed starting coordinate of (0,0) for first task.
                starting_coords = (0, 0)
                prev_end_time = 0
            else:   # i >= 1
                starting_coords = self.tasks[i-1].coords
                prev_end_time = self.tasks[i-1].end_time
            # Generate frames for task
            self.tasks[i].generateFrames(self.speed, frame_rate, starting_coords, prev_end_time)
            self.frames = self.frames + self.tasks[i].frames
...
class Task(object):
    ...
    def generateFrames(self, speed, frame_rate, starting_coords, prev_end_time):
        ...
        dubins_frames, _ = self.path.path.sample_many(distance_per_frame)
        self.frames = self.frames + dubins_frames

        # Add frames for the duration of the task, where the agent remains at the
        # task coordinates.
        # round() - number of frames needs to be an integer. Rounding may cause the animation to be misrepresentative of the actual
        # solution plan. See the comment above about using round() in the generation of frames.
        for i in range(round(frame_rate * self.duration)):
...
class BaseSolver(object):
    ...
    def associate(self):
        ...
        for agent_id in self.plan.keys():
            ...
            # 'task_tuple' is a tuple of the form (task_id, [prereqs. (= task IDs)], heading angle).
            # For each task...
            for task_tuple in self.plan[agent_id]:
                task_id = task_tuple[0]
                task = self.tasks[task_id]

                # Ignore the FINISH task. It is (possibly, conditional on changes in implementation)
                # assigned to an agent by the GA, but causes problems if it is not filtered
                # out. Filter out the INIT task as well.
                if (task.type == Tasks.FINISH) or (task.type == Tasks.INIT):
                    continue    # Don't assign to an agent.

                prereqs = task_tuple[1]
                angle = task_tuple[2]
